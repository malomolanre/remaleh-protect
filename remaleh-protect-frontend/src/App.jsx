...
function App() {
  ...
  const handleScamCheck = async (e) => {
    ...
    try {
      ...
      // 2. Breach Check (only if emails found) – call the backend API
      if (emails.length > 0) {
        servicePromises.push(
      ...
      // 4. Enhanced Local Analysis (Comprehensive)
      serviceResults.localAnalysis = performEnhancedLocalAnalysis(scamMessage, urls, emails);

      // Wait for all service calls (scam detection, breach check, link analysis) to complete
  ...
  const aggregateScamResults = (serviceResults, urls, emails, startTime) => {
    ...
    if (serviceResults.linkAnalysis && !serviceResults.linkAnalysis.error) {
      ...
      // Use the average risk score from the backend to influence totalScore (scaled)
      if (typeof analysis.average_risk_score === 'number') {
        totalScore += analysis.average_risk_score;
      }
      servicesUsed.push('✓ Link Analysis');
      serviceDetails.push(`Link Analysis: ${urlsAnalysed.length || urls.length} URL(s) analysed`);
      // Pull indicators from each analysed URL
      ...
      servicesUsed.push('✗ Link Analysis (unavailable)');
    }
    ...
    // Create a plain-language summary for readability
    let simpleSummary = '';
    if (totalScore >= 70) {
      simpleSummary = '⚠️ This message looks like a scam. It contains multiple warning signs such as a suspicious link and urgent language.';
    } else if (totalScore >= 40) {
      simpleSummary = '⚠️ This message appears suspicious. It has several signs that it might be a scam.';
    } else if (totalScore >= 15) {
      simpleSummary = '⚠️ Be cautious. This message has some risky elements and could be a scam.';
    } else {
    ...
    return `
      <div style="padding: 20px; border-radius: 8px; background: #f8fafc; border-left: 4px solid ${riskColor};">
        ...
        <!-- Simple summary for families and elderly -->
        ...
        ${topIndicators.length > 0 ? `
          <div style="margin: 12px 0;">
            ...
            <div style="color: #b91c1c; font-size: 13px; line-height: 1.4;">
              ...
              ${allIndicators.length > topIndicators.length ? `<br><em>...and more warning signs detected</em>` : ''}
            </div>
          </div>
        ` : ''}
        ...
        <!-- Detailed analysis section (optional) -->
  ...
  const handleBreachCheck = async (e) => {
    ...
    try {
      // Make real API call to check breaches
      ...
      setBreachResult({ error: true, message: 'Unable to check breaches at this time.' });
    } finally {
      setIsChecking(false);
    }
  };
